...
TOPOLOGY_SPREAD_TEMPLATE = {'topologySpreadConstraints': [{'maxSkew': 1, 'topologyKey': 'kubernetes.io/hostname', 'whenUnsatisfiable': 'DoNotSchedule', 'labelSelector': {'matchLabels': {'app': ''}}}]}
...
def create_deployment_service_yaml_files(workmodel, k8s_parameters, nfs, output_path):
    ...
    for service in workmodel:
        ...
        with open(f"{K8s_YAML_BUILDER_PATH}/Templates/DeploymentTemplate.yaml", "r") as file:
            ...
            # Handle topology spread constraints (takes precedence over antiaffinity)
            if "pod_topology_spread" in workmodel[service].keys() and workmodel[service]['pod_topology_spread']==True:
                TOPOLOGY_TO_ADD = TOPOLOGY_SPREAD_TEMPLATE.copy()
                TOPOLOGY_TO_ADD['topologySpreadConstraints'][0]['labelSelector']['matchLabels']['app'] = service
                if "max_skew" in workmodel[service].keys():
                    TOPOLOGY_TO_ADD['topologySpreadConstraints'][0]['maxSkew'] = workmodel[service]['max_skew']
                TOPOLOGY_TO_ADD = str(yaml.dump(TOPOLOGY_TO_ADD)).replace('\n','\n      ').rstrip()
                f = f.replace("{{TOPOLOGY_SPREAD}}", TOPOLOGY_TO_ADD)
                ...
                f = f.replace("{{TOPOLOGY_SPREAD}}", "".rstrip())
            else:
                f = f.replace("{{POD_ANTIAFFINITY}}", "".rstrip())
                f = f.replace("{{TOPOLOGY_SPREAD}}", "".rstrip())
