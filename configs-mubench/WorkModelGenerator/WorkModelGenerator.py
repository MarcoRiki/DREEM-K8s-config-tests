...
def get_work_model(service_graph, workmodel_params):
    ...
    for k in workmodel_params.keys():
        ...
        if "pod_antiaffinity" in w["value"]:
            tmp_dict.update({"pod_antiaffinity": w["value"]["pod_antiaffinity"]})
        if "pod_topology_spread" in w["value"]:
            tmp_dict.update({"pod_topology_spread": w["value"]["pod_topology_spread"]})
        if "max_skew" in w["value"]:
            tmp_dict.update({"max_skew": w["value"]["max_skew"]})
